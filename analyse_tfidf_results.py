...
def main():
    ...
    for fname in os.listdir(args.candidates):
        ...
        ref_name = fname.replace("class_candidates_tfidf", "class_reference").replace(
            "predicate_candidates_tfidf", "predicate_reference"
        )
    ...
    if results:
        ...
        out_df.to_csv(
            os.path.join(args.output, "aggregate_hits_at_k_tfidf.csv"), index=False
        )
